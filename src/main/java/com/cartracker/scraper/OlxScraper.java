...
public class OlxScraper {
  ...
  public List<ScrapeResponse> fetchCars() throws IOException, InterruptedException {
    if (delayMillis > 0) {
    ...
    for (Element card : cards) {
      try {
        ScrapeResponse r = parseCard(card);
        if (r == null) {
  ...
  private ScrapeResponse parseCard(Element card) {
    String href = card.attr("href");
    String externalId = href.replace(
        "/artikal/",
        ""
    ).trim();
